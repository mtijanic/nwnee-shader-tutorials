...
vec3 gaussian_blur_vertical(in vec2 xy, in sampler2D tex)
{
    ...
    b += texture2D(tex, xy + vec2(0.0, gblur_offsets[0])/screenHeight).rgb * gblur_weight[1];
    b += texture2D(tex, xy - vec2(0.0, gblur_offsets[0])/screenHeight).rgb * gblur_weight[1];
    b += texture2D(tex, xy + vec2(0.0, gblur_offsets[1])/screenHeight).rgb * gblur_weight[2];
    b += texture2D(tex, xy - vec2(0.0, gblur_offsets[1])/screenHeight).rgb * gblur_weight[2];
...
vec3 gaussian_blur_horizontal(in vec2 xy, in sampler2D tex)
{
    ...
    b += texture2D(tex, xy + vec2(gblur_offsets[0], 0.0)/screenHeight).rgb * gblur_weight[1];
    b += texture2D(tex, xy - vec2(gblur_offsets[0], 0.0)/screenHeight).rgb * gblur_weight[1];
    b += texture2D(tex, xy + vec2(gblur_offsets[1], 0.0)/screenHeight).rgb * gblur_weight[2];
    b += texture2D(tex, xy - vec2(gblur_offsets[1], 0.0)/screenHeight).rgb * gblur_weight[2];
